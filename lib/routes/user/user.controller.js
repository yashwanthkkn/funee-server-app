...
router.get('/home/rc',(req,res)=>{
    Topic.getTopicsAsRecent((err,topics)=>{
        if(err){
            res.render('recentTopics',{user:req.user,topics:null})
        }else{
            res.render('recentTopics',{user:req.user,topics:topics})
        }
...
router.get('/admin/msg',(req,res)=>{
    if(req.user.claim == 'admin'){
        Topic.getReportComments({},(err,comments)=>{
            res.render('adminMsg',{comments:comments,user:req.user})
        })
    }

})

router.get('/admin/removeMsg/:uuid',(req,res)=>{
    var uuid = req.params.uuid;
    var data = {};
    data.uuid = uuid;
    if(uuid && req.user.claim == 'admin'){
        Topic.deleteComment(data,(err)=>{
            res.redirect('/user/admin/msg')
        })
    }
})
